...
try{
  ...
  document.write('<script src="glide-services.js?v=20260912-40"><\/script>');
...
function mergeGlideServices(){
  if(!Array.isArray(window.__dabbarliGlideServices))return;
  if(!Array.isArray(window.__dabbarliPublicServices))window.__dabbarliPublicServices=[];
  var ids=new Set(window.__dabbarliPublicServices.map(function(s){return String(s.id)}));
  window.__dabbarliGlideServices.forEach(function(s){if(!ids.has(String(s.id))){window.__dabbarliPublicServices.push(s);ids.add(String(s.id))}});
}
setTimeout(mergeGlideServices,0);
function wrapSearch(){
  if(typeof window.searchPage!=='function'){setTimeout(wrapSearch,50);return}
  if(window.searchPage.__public40)return;
  ...
  window.searchPage.__public40=true;
}
wrapSearch();
})();